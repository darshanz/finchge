...
def test_raises_error_without_genotype():
    ...
    mutation = GaussianMutation(
        mutation_probability=0.5, codon_size=127, random_state=42
    )
...
def test_probability_out_of_range_raises():
    ...
    with pytest.raises(ValueError):
        GaussianMutation(mutation_probability=2.0, codon_size=127)
...
def test_does_not_modify_parent():
    ...
    mutation = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=5.0, random_state=42
    )
    mutation.mutate(ind)
...
def test_genome_length_preserved():
    ...
    mutation = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=10.0, random_state=42
    )
...
def test_values_are_non_negative():
    ...
    mutation = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=100.0, random_state=42
    )
    child = mutation.mutate(ind)
...
def test_zero_probability_no_change():
    ...
    ind = make_individual(genome.copy())
    mutation = GaussianMutation(
        mutation_probability=0.0, codon_size=127, random_state=42
    )
    child = mutation.mutate(ind)
...
def test_zero_std_dev_preserves_values_when_selected():
    ...
    ind = make_individual(genome.copy())
    mutation = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=0.0, random_state=42
    )
    child = mutation.mutate(ind)
    assert child.genotype == genome
...
def test_large_std_dev_changes_values():
    ...
    ind = make_individual(genome.copy())
    mutation = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=1000.0, random_state=42
    )
    child = mutation.mutate(ind)
    assert child.genotype != genome
...
def test_deterministic_with_fixed_seed():
    ...
    m1 = GaussianMutation(
        mutation_probability=0.5, codon_size=127, std_dev=5.0, random_state=7
    )
    m2 = GaussianMutation(
        mutation_probability=0.5, codon_size=127, std_dev=5.0, random_state=7
    )
...
def test_higher_std_dev_produces_larger_deviations():
    ...
    m_low = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=1.0, random_state=42
    )
    m_high = GaussianMutation(
        mutation_probability=1.0, codon_size=127, std_dev=50.0, random_state=42
    )
