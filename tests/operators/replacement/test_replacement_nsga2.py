...
def test_nsga2_replacement_skips_unranked_old_individuals_when_selecting_elites():
    unranked_old = make_individual([3.0, 3.0])
    ranked_old = make_individual([2.0, 2.0], rank=0)
    child = make_individual([1.0, 1.0], rank=0)

    replacement = NSGA2ElitistReplacement(maximize_flags=[False, False])

    selected = replacement.replace(
