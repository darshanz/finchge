...
from finchge.operators.crossover import TwoPointCrossover
...
def test_cross_raises_error_if_genotype_missing():
    ...
    crossover = TwoPointCrossover(codon_size=8, crossover_proba=1.0, random_state=42)
...
def test_cross_returns_two_offspring():
    ...
    parent1 = make_individual([1, 2, 3, 4, 5], used_codon_count=5)
    parent2 = make_individual([6, 7, 8, 9, 10], used_codon_count=5)
    crossover = TwoPointCrossover(codon_size=8, crossover_proba=1.0, random_state=42)
...
def test_no_crossover_when_probability_zero():
    ...
    parent1 = make_individual([1, 2, 3, 4, 5], used_codon_count=5)
    parent2 = make_individual([6, 7, 8, 9, 10], used_codon_count=5)
    crossover = TwoPointCrossover(codon_size=8, crossover_proba=0.0, random_state=42)
    child1, child2 = crossover.cross(parent1, parent2)
    assert child1.genotype == parent1.genotype
    assert child2.genotype == parent2.genotype


def test_two_point_crossover_swaps_middle_segments_when_probability_one():
    # when crossover probability is one,
    # offspring are created by exchanging middle genome segments.
    parent1 = make_individual([1, 2, 3, 4, 5, 6], used_codon_count=6)
    parent2 = make_individual([7, 8, 9, 10, 11, 12], used_codon_count=6)
    crossover = TwoPointCrossover(codon_size=8, crossover_proba=1.0, random_state=1)
    child1, child2 = crossover.cross(parent1, parent2)
    assert child1.genotype != parent1.genotype or child2.genotype != parent2.genotype


def test_crossover_respects_within_used_true():
    #  when within_used is True, crossover points
    # are restricted to the used portion of the genomes.
...
def test_crossover_allows_full_genome_when_within_used_false():
    #  when within_used is False, crossover points
    # may occur anywhere within the full genome length.
...
def test_crossover_requires_minimum_two_valid_cut_points():
    # if there are not enough valid cut points
    # (less than two), crossover does not occur and parents are copied.
    parent1 = make_individual([1, 2], used_codon_count=1)
    parent2 = make_individual([3, 4], used_codon_count=1)
    crossover = TwoPointCrossover(codon_size=8, crossover_proba=1.0, random_state=42)
    child1, child2 = crossover.cross(parent1, parent2)
    assert child1.genotype == parent1.genotype
    assert child2.genotype == parent2.genotype
...
def test_crossover_is_deterministic_with_fixed_random_state():
    ...
    parent1 = make_individual([1, 2, 3, 4, 5, 6], used_codon_count=6)
    parent2 = make_individual([7, 8, 9, 10, 11, 12], used_codon_count=6)
    crossover1 = TwoPointCrossover(codon_size=8, crossover_proba=1.0, random_state=123)
    crossover2 = TwoPointCrossover(codon_size=8, crossover_proba=1.0, random_state=123)
